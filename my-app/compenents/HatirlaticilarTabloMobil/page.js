...
import HatirlaticilarOptions from '../HatirlaticilarOptions/page'
...
export default function Page ({hatirlaticilar}){
    ...
    const [selectedHatirlaticilarItem,setSelectedHatirlaticilarItem] = useState(null)
    ...
    const deleteHandle = async (id) => {
        ...
        const data = await deleteHatirlaticilar(id);
        ...
        if (data) {
            ...
            const newTotalPages = Math.max(
                1,
                Math.ceil((hatirlaticilar.count - 1) / itemsPerPage)
            );
            ...
            setSelectedHatirlaticilarItem(null)
    ...
    const openOptions = (item) => {
        setSelectedHatirlaticilarItem(item);
    };
    ...
    return(
        <div className={styles.tableContainer}>
            <div className={styles.mainContainer}>
                    ...
                    <div key={item.id ?? index} className={styles.elemanContainer} onClick={()=>openOptions(item)}>
                        <div className={styles.elemanLeft}>
                            <div className={styles.elemanText}>{item.arac_no}</div>
                            <div className={styles.elemanText}>{item.hatirlatma_turu}</div>
                        </div>
            ...
            {selectedHatirlaticilarItem && <HatirlaticilarOptions item={selectedHatirlaticilarItem} setOptions={setSelectedHatirlaticilarItem} deleteHandle={deleteHandle}/>}
        </div>
    )
}